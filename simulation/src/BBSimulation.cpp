/**
 * Copyright (c) 2019-2020. Loïc Pottier.
 *
...
XBT_LOG_NEW_DEFAULT_CATEGORY(bb_simulation,
"Log category for BB Simulation");
...
BBSimulation::parseFilesList(std::string path, std::shared_ptr <wrench::StorageService> pfs_service,
                             std::shared_ptr <wrench::StorageService> bb_service) {
    ...
    if (stage_list_file.is_open()) {
        while (std::getline(stage_list_file, line)) {
            ...
            if (src_path.find_first_of(bb_prefix_cori) == std::string::npos ||
                src_path.find_first_of(bb_prefix_summit) == std::string::npos) {
                std::cerr << "[ERROR] file: " << src_file << " cannot be in burst buffers before being staged in"
                          << std::endl;
                std::exit(1);
            }

            if (src_file != dst_file && dst_file.length() > 0) {
                std::cerr << "[ERROR] file: " << src_file << " is different from destination file: " << dst_file
                          << std::endl;
                std::exit(1);
            }

            if (dst_path.find_first_of(bb_prefix_cori) != std::string::npos ||
                dst_path.find_first_of(bb_prefix_summit) != std::string::npos) {
...
wrench::Workflow *BBSimulation::parse_inputs() {
    ...
    if (ends_with(this->raw_args["workflow_file"], "dax")) {
        this->workflow = wrench::PegasusWorkflowParser::createWorkflowFromDAX(raw_args["workflow_file"],
                                                                              flop_rate + "f");
    } else if (ends_with(this->raw_args["workflow_file"], "json")) {
        this->workflow = wrench::PegasusWorkflowParser::createWorkflowFromJSON(raw_args["workflow_file"],
                                                                               flop_rate + "f");
    } else {
        std::cerr << "Workflow file: " << raw_args["workflow_file"] << "name must end with '.dax' or '.json'"
                  << std::endl;
        exit(1);
    }
...
std::pair<double, double> BBSimulation::check_links(std::map<std::pair < std::string, std::string>, simgrid::s4u::Link *
...
res.
first = first_elem.second->get_bandwidth();
res.
second = first_elem.second->get_latency();
...
if (res.first != elem.second->

get_bandwidth()

) {
std::cerr << "Bandwidth issue in the platform XML. The same bandwidth must be used for every similar links." <<
std::endl;
exit(1);
}

if (res.second != elem.second->

get_latency()

) {
std::cerr << "Latency issue in the platform XML. The same latency must be used for every similar links." <<
std::endl;
exit(1);
}
...
BBSimulation::instantiate_storage_services() {
    ...
    std::map<std::string, double> pfs_payload = {
            ...
            {wrench::SimpleStorageServiceMessagePayload::FILE_WRITE_REQUEST_MESSAGE_PAYLOAD,  10000}, //latest res: 100000
            {wrench::SimpleStorageServiceMessagePayload::FILE_WRITE_ANSWER_MESSAGE_PAYLOAD,   10000}, //latest res: 100000
            {wrench::SimpleStorageServiceMessagePayload::FILE_READ_REQUEST_MESSAGE_PAYLOAD,   50000}, //latest res: 100000
            {wrench::SimpleStorageServiceMessagePayload::FILE_READ_ANSWER_MESSAGE_PAYLOAD,    50000}, //latest res: 100000
    };
    ...
    std::map<std::string, double> bb_payload = {
            ...
            {wrench::SimpleStorageServiceMessagePayload::FILE_COPY_REQUEST_MESSAGE_PAYLOAD,   0},
            {wrench::SimpleStorageServiceMessagePayload::FILE_COPY_ANSWER_MESSAGE_PAYLOAD,    0},
            {wrench::SimpleStorageServiceMessagePayload::FILE_WRITE_REQUEST_MESSAGE_PAYLOAD,  16000}, // latest res: 120000000
            {wrench::SimpleStorageServiceMessagePayload::FILE_WRITE_ANSWER_MESSAGE_PAYLOAD,   16000}, // latest res: 120000000
            {wrench::SimpleStorageServiceMessagePayload::FILE_READ_REQUEST_MESSAGE_PAYLOAD,   8000}, // latest res: 120000000
            {wrench::SimpleStorageServiceMessagePayload::FILE_READ_ANSWER_MESSAGE_PAYLOAD,    8000}, // latest res: 120000000
    };
...
std::set <std::shared_ptr<wrench::ComputeService>> BBSimulation::instantiate_compute_services() {
    ...
    std::map<std::string, double> compute_payload_values = {
            ...
            {wrench::ComputeServiceMessagePayload::SUBMIT_STANDARD_JOB_REQUEST_MESSAGE_PAYLOAD,    15000}, //latest res: 3000000000
    ...
    try {
        ...
        for (auto host : this->execution_hosts) {
            compute_resources[host] = std::make_tuple(wrench::ComputeService::ALL_CORES,
                                                      wrench::ComputeService::ALL_RAM);

            auto baremetal_service = new wrench::BareMetalComputeService(
                    host, compute_resources, {"/"}, {}, compute_payload_values);
...
void BBSimulation::dumpAllOutputJSON(bool layout) {
    ...
    simulation_output.dumpWorkflowExecutionJSON(this->workflow, this->raw_args["output_dir"] + "/" + this->workflow_id +
                                                                "-execution.json", false);

    if (layout)
        simulation_output.dumpWorkflowExecutionJSON(this->workflow,
                                                    this->raw_args["output_dir"] + "/" + this->workflow_id +
                                                    "-execution-layout.json", true);

    simulation_output.dumpWorkflowGraphJSON(this->workflow,
                                            this->raw_args["output_dir"] + "/" + this->workflow_id + ".json");
}
...
void BBSimulation::dumpResultPerTaskCSV(const std::string &output, char sep) {
    ...
    std::cout << "Task in first trace entry: " << stage_in->getID() << " " << stage_in->getStartDate() << " "
              << stage_in->getEndDate() << " " << stage_in->getEndDate() - stage_in->getStartDate() << std::endl;
    std::cout << "Task in last trace entry: " << stage_out->getID() << " " << stage_out->getStartDate() << " "
              << stage_out->getEndDate() << " " << stage_out->getEndDate() - stage_out->getStartDate() << std::endl;
}
