...
class SwarpInstance:
    ...
    def script_run_resample(self):
        ...
        string += "    cd ${process}\n"
        string += "    srun -N 1 -n 1 -c ${CORES_PER_PROCESS} -o \"output.resample.%j.${process}\" -e \"error.resample.%j.${process}\" $LAUNCH $EXE -c $RESAMPLE_CONFIG ${indir}/${IMAGE_PATTERN} & \n"
        string += "    cd ..\n"
        string += "done\n"
    ...
    def script_run_combine(self):
        ...
        string += "    cd ${process}\n"
        string += "    srun -N 1 -n 1 -c ${CORES_PER_PROCESS} -o \"output.coadd.%j.${process}\" -e \"error.coadd.%j.${process}\" $LAUNCH $EXE -c -c $COMBINE_CONFIG ${RESAMPLE_PATTERN} &\n"
        string += "    cd ..\n"
        string += "done\n"
        string += "\n"
        # string += "sleep 10\n"
